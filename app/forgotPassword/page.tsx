...
import { ForgotPasswordApi, ResetPassword } from '@/service/request';
...
export default function ForgotPasswordPage() {
    ...
    const handleEmailSubmit = async (isResend = false) => {
        ...
        try {
            const res = await ForgotPasswordApi.FETCH(formData);
            if (res.data.success !== '') {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            const res = await ResetPassword.UPDATE(formData);
            console.log(res)
            if (res.data.data !== '') {
    ...
    return (
        <div className="flex justify-center items-center h-[400px]">
            <div className="max-w-md w-full mx-auto bg-white p-8 border border-gray-300 shadow-lg rounded-lg">
                {page === 0 && (
                    <Form onFinish={onEmailFormFinish}>
                        ...
                        <Form.Item>
                            <CustomButton
                                children='Send OTP'
                                htmlType='submit'
                                addedClass='bg-sky-600 w-full text-white'
                    ...
                    <Form onFinish={handleOtpSubmit}>
                        ...
                        <Form.Item>
                            <CustomButton
                                children='Resend OTP'
                                htmlType='button'
                                addedClass='bg-gray-600 w-full text-white'
