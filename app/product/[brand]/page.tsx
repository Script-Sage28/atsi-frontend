...
export default function Productpage({ params }: { params: { brand: string } }) {
  ...
  const onLoadMore = () => {
    setLoading(true);
    const nextItems = product.list?.slice(
      list.length,
      list.length + countPerPage,
    );
  ...
  useEffect(() => {
    if (selectedBrands) {
      const cat = shopby.brand?.filter(
        (data: { id: string }) => data.id === selectedBrands,
      );
      console.log(cat);
      if (cat.length > 0) {
        console.log(cat);
  ...
  useEffect(() => {
    const list = filteredAndSortedProducts?.map((item: any) => ({
      ...item,
      loading: false,
    }));
    setList(list);
  ...
  const filteredAndSortedProducts = product.list
                ...
                <CustomLabel
                  children={
                    selectedBrand.length > 0
                      ? `${selectedBrand[0].name} Categories`
                      : 'All Categories'
                  }
                  variant="text"
            ...
            <div className="w-full justify-end items-end flex flex-col relative  mb-2 h-max">
              ...
              <div className="w-full flex gap-4 md:gap-12 items-start flex-wrap px-2 md:pr-12 md:pl-8 mb-4">
                <div className="flex-1 flex-col md:flex-row flex md:items-center gap-4">
                  <label
                    className="whitespace-nowrap h-full text-[20px]"
                    htmlFor=""
                  >
                    Shop by:
                  </label>
              ...
              {selectedBrand?.length > 0 &&
                selectedBrand?.map((item: T_Brand, idx) => (
                  <div
                    key={idx}
                    className="w-full md:w-11/12 flex flex-col items-start flex-wrap md:ml-12 pt-8 pb-4 border-b-2 border-gray-400 mb-4"
                  >
                    <CustomLabel
                      children={item.name}
                      variant="text"
              ...
              {list?.length > 0 ? (
                <div
                  className={clsx(
                    ...
                    'w-full flex flex-wrap gap-4 md:p-8 justify-stretch items-normal md:justify-normal',
                  )}
                >
                  {isRow ? (
                    <List
                      ...
                      renderItem={(data: any, idx: number) => (
                        <List.Item>
                          <div
                            key={idx}
                            className={clsx(
                              'w-full shadow-border rounded-t-lg hover:shadow-shine bg-gray-200 h-max pb-4 cursor-pointer',
                            )}
                          >
                            <Skeleton
                            ...
                            >
                              <div className="w-full min-w-full min-h-[140px] flex justify-center items-center relative">
                                ...
                                {data.status !== 'Available' && (
                                  <div className="absolute px-4 py-2 bg-black text-white w-max">
                                    <p>
                      ...
                      renderItem={(data: any, idx: number) => (
                        <List.Item>
                          <Link
                            ...
                            key={idx}
                            className={clsx(
                              'w-[500px] md:w-full h-max shadow-border flex flex-col md:flex-row rounded-t-lg hover:shadow-shine bg-gray-200 cursor-pointer',
                            )}
                          >
                            <Skeleton
                              style={{ padding: 8, height: '200px' }}
                              ...
                              <div className="p-4 hover:bg-white flex flex-col md:flex-row gap-2 w-full">
                                ...
                                <CustomLabel
                                  children={
                                    data.discountedPrice &&
                                    data.discountedPrice !== 0 ? (
                                      <div className="flex gap-8\2 flex-col">
                                        <p className="m-0">
                                          {Peso(data.discountedPrice)}
                                        </p>
                                        <p className="m-0 line-through text-gray-600">
