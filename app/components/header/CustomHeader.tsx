...
export default function CustomHeader() {
  ...
  const content = (
    <div>
      <p onClick={showModal} className="p-2 cursor-pointer">
        Edit details
      </p>
      <p onClick={hanldeLogout} className="p-2 cursor-pointer">
        Logout
      </p>
    </div>
  );
  ...
  return (
    <>
      <Header className="header sticky h-20 text-[14px] lg:text-[18px] lg:h-24 z-50 top-0 p-2 md:p-8 bg-white w-full flex justify-between items-center">
        <div className="flex-1 w-full flex justify-between items-center">
          {open && (
            <MdOutlineClear
              size={40}
              className="md:hidden ease-in-out cursor-pointer"
      ...
      <div
        className={clsx(
          !open
            ? 'hidden h-0 transform scale-y-0'
            : 'z-50 h-max w-full bg-zinc-300 absolute top-16 overflow-hidden transition-transform transform scale-y-100 ease-in-out duration-300',
        )}
      >
        ...
        <div className="flex flex-wrap flex-col gap-4 px-4 pb-12 pt-4  md:hidden">
          {links?.map((link, idx) => (
            <Link
              href={link.url}
              className="font-semibold flex-grow text-gray-500 hover:text-white hover:bg-zinc-500 p-4 rounded-md"
              key={idx}
            >
              {link.name}
            </Link>
